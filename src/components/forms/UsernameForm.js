...
export default function UsernameForm({ desiredUsername, lang = 'mk' }) {
  ...
  return (
    <form onSubmit={handleSubmit}>
      <h1 className="text-4xl font-bold text-center mb-2">{t.usernameFormTitle}</h1>
      <div className="max-w-xs mx-auto">
        <input
          name="username"
          className="block p-2 mx-auto text-center border w-full mb-2"
          defaultValue={desiredUsername}
          type="text" placeholder="username / корисничко иme" />
        {taken && (
          <div className="bg-[#fecaca] border border-[#ef4444] p-2 mb-2 text-center">
            {t.usernameTaken}
          </div>
        )}
        <button type="submit" className="bg-[#3b82f6] text-white py-2 px-4 block mx-auto w-full border-2 hover:bg-transparent hover:text-[#2563eb] hover:border-2 border-[#3b82f6]">{t.usernameSave}</button>
      </div>
    </form>
  )
}
